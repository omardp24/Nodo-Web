...
[src/gmail/](src/gmail/) revisa **todas las cuentas de Gmail conectadas** cada 5 minutos (`@Cron(CronExpression.EVERY_5_MINUTES)` en [src/gmail/gmail-ingest.service.ts](src/gmail/gmail-ingest.service.ts)) y crea un `Recordatorio` con `origen: 'CORREO'` por cada correo nuevo en la bandeja **Principal** de cada cuenta que el asistente clasifique como accionable (asunto → `titulo`; `descripcion` es el **resumen** de la acción pendiente que arma Gemini, no el snippet crudo del correo — ver "Filtro por LLM" abajo). Si el correo menciona una fecha/plazo concreto, `fechaLimite` se completa con lo que extraiga Gemini; si no, queda sin definir y se ajusta a mano (y entra al ciclo de notificaciones repetidas — ver Notificaciones push). El frontend muestra los recordatorios sin fecha en una sección "Sin fecha" aparte (ver `nodo-app/src/components/nodo/proximos-list.tsx`) — de lo contrario quedan invisibles, porque el resto de las vistas está organizado por fecha.
...
- **Gestionar cuentas desde la app**: `GmailCuentasController` (`GET /gmail/cuentas`, `PATCH /gmail/cuentas/:id`, `DELETE /gmail/cuentas/:id`, protegido por el guard normal — a diferencia de `/auth/gmail*` que es público) expone las cuentas conectadas al frontend (sin el `refreshToken`, vía `GmailAccountsService.listarPublico()`) para listarlas, reasignarlas de Lista y desconectarlas desde Perfil. Conectar una cuenta nueva sigue siendo un flujo de navegador aparte (`/auth/gmail`), no algo que se pueda hacer solo con estos endpoints.
- **A qué Lista van los correos de cada cuenta**: por defecto, todas las cuentas caen en la Lista compartida "Correos" (una Categoria por cuenta ahí adentro). `CuentaGmail.listaId` (nullable, `PATCH /gmail/cuentas/:id { listaId }`) permite mandar una cuenta a **cualquier otra Lista** en su lugar — ej. la personal a "Personal", la corporativa a "Trabajo" — para que sus correos queden mezclados con el resto de los nodos de esa Lista en vez de aislados en "Correos". `listaId: null` la vuelve a la Lista por defecto. Es solo hacia adelante: reasignar una cuenta no mueve los recordatorios que ya se crearon bajo la Categoria vieja. `GmailIngestService.getCategoriaId()` cachea la Categoria resuelta por `${email}:${listaId}` (no solo por email), así que una reasignación se nota en el siguiente ciclo sin reiniciar el server. `onDelete: SetNull` en el schema — borrar la Lista elegida no borra la cuenta conectada, solo la vuelve a "Correos".
- **Filtro por LLM (`accionable`) + fecha + resumen, en UNA sola llamada**: `category:primary` deja pasar bastante ruido (notificaciones informativas, confirmaciones de algo ya resuelto, correos "normales" sin nada pendiente), así que antes de crear el Recordatorio se llama a `AsistenteService.clasificarCorreo(asunto, snippet, fechaRecepcion, zonaHoraria?)` (mismo cliente Gemini que Vínculo, ver sección Asistente Vínculo), que devuelve `{ accionable: boolean, fechaLimite?: string, resumen?: string }` con salida JSON forzada por schema. Si `accionable` es `false` el correo se descarta (no se crea Recordatorio) pero **igual se marca con el label `Recordatorio-creado`**, para no volver a evaluarlo en cada ciclo. Si viene `fechaLimite` (el correo menciona un plazo concreto, ej. "tienes hasta mañana a las 9am"), se resuelve contra la fecha en que **llegó el correo** (`message.internalDate`, no "ahora") en la zona horaria de `GMAIL_ZONA_HORARIA` (`.env` — no hay per-cuenta ni per-usuario, es una app personal). `resumen` es una frase corta y directa de qué hay que hacer (ej. "Enviar el informe antes de las 9am") pensada para la notificación push — reemplaza al snippet crudo del correo como `descripcion` del Recordatorio (con el snippet como respaldo si Gemini no lo devuelve). Todo esto es deliberadamente la MISMA llamada a Gemini que antes solo devolvía `accionable` — agregar fecha y resumen no duplica el costo por correo. Si `GEMINI_API_KEY` no está configurada o la llamada a Gemini falla, devuelve `{ accionable: true }` (sin fecha ni resumen) por defecto (fail-safe: se prefiere un falso positivo — un recordatorio de más que hay que borrar a mano — a perder silenciosamente un correo que sí requería acción).
...
- **Disparo (nodos CON fecha, una sola vez)**: `RecordatoriosNotificadorService.notificarVencidos()` ([src/notificaciones/recordatorios-notificador.service.ts](src/notificaciones/recordatorios-notificador.service.ts)) corre cada minuto (`@Cron(CronExpression.EVERY_MINUTE)`) y busca `Recordatorio` con `estado: PENDIENTE`, `notificadoEn: null` y `fechaLimite` ya cumplida; por cada uno, notifica a **todas** las suscripciones guardadas (no hay `userId` en el modelo, ver sección Autenticación) y marca `notificadoEn`. Acá `notificadoEn` es la deduplicación — sin `userId` para filtrar, es lo único que evita reenviar el mismo aviso en cada tick.
- **Disparo (correos SIN fecha, repetido)**: `recordarPendientesSinFecha()`, en el mismo service, corre cada 5 minutos y busca `Recordatorio` con `estado: PENDIENTE`, `origen: 'CORREO'`, `fechaLimite: null` y `notificadoEn` nulo o más viejo que `RECORDATORIO_REPETIR_HORAS` (`.env`, default 4h). Un correo accionable (ver Ingesta de correos) puede no traer una fecha explícita — sin este recordatorio periódico, una sola notificación (o ninguna, si se creó fuera de horario) es fácil de perder de vista para siempre. Acá `notificadoEn` significa "última vez que se avisó", no "ya se avisó alguna vez" — se sigue re-notificando mientras el nodo siga `PENDIENTE`; se detiene solo al completarlo (deja de matchear el `where`) o al asignarle una `fechaLimite` (pasa a ser responsabilidad de `notificarVencidos` en su lugar, una sola vez).
- **`RecordatoriosService.update()` resetea `notificadoEn` al cambiar `fechaLimite`**: si no lo hiciera, un nodo ya notificado (vencido, o que llevaba rato en el ciclo repetido de arriba) que recibe una nueva fecha (ej. "posponer" desde el frontend) quedaría mudo para siempre, porque `notificarVencidos` solo mira `notificadoEn: null`.
