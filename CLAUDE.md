...
- **Filtro por LLM (`accionable`) + extracción de fecha, en UNA sola llamada**: `category:primary` deja pasar bastante ruido (notificaciones informativas, confirmaciones de algo ya resuelto, correos "normales" sin nada pendiente), así que antes de crear el Recordatorio se llama a `AsistenteService.clasificarCorreo(asunto, snippet, fechaRecepcion, zonaHoraria?)` (mismo cliente Gemini que Vínculo, ver sección Asistente Vínculo), que devuelve `{ accionable: boolean, fechaLimite?: string }` con salida JSON forzada por schema. Si `accionable` es `false` el correo se descarta (no se crea Recordatorio) pero **igual se marca con el label `Recordatorio-creado`**, para no volver a evaluarlo en cada ciclo. Si viene `fechaLimite` (el correo menciona un plazo concreto, ej. "tienes hasta mañana a las 9am"), se resuelve contra la fecha en que **llegó el correo** (`message.internalDate`, no "ahora") en la zona horaria de `GMAIL_ZONA_HORARIA` (`.env` — no hay per-cuenta ni per-usuario, es una app personal) y se guarda directo en el Recordatorio. Deliberadamente es la MISMA llamada a Gemini que antes solo devolvía `accionable` — agregar la extracción de fecha no duplica el costo por correo. Si `GEMINI_API_KEY` no está configurada o la llamada a Gemini falla, devuelve `{ accionable: true }` (sin fecha) por defecto (fail-safe: se prefiere un falso positivo — un recordatorio de más que hay que borrar a mano — a perder silenciosamente un correo que sí requería acción).
